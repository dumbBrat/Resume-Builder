...
# Register custom font (replace with your chosen .ttf path if needed)
pdfmetrics.registerFont(TTFont("Roboto", "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"))
...
def build_minimal_template(buffer, data):
    ...
    # --- Our custom styles (unique names, no clashes) ---
    styles.add(ParagraphStyle(name="MyHeader", fontName="Roboto", fontSize=18, leading=22, spaceAfter=8, textColor=colors.black))
    styles.add(ParagraphStyle(name="MySectionHeader", fontName="Roboto", fontSize=12, leading=16, spaceAfter=6, textColor=colors.black, spaceBefore=12))
    styles.add(ParagraphStyle(name="MyBody", fontName="Roboto", fontSize=10, leading=14, spaceAfter=4))
    styles.add(ParagraphStyle(name="MyBullet", fontName="Roboto", fontSize=10, leading=12, leftIndent=15, bulletIndent=5))
